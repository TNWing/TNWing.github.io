...
// modifications to the toy
// changed bead colour on click to random
// changed click sound to bucket
// changed bead shape to circle
// changed grid background to teal
// changed status text
// changed status text colour to blue
// changed grid size to 20x10
// added random glyphs to beads
// changed cover to personal cover
...
PS.init = function( system, options ) {
	...
	PS.gridColor( 0xB0FFFC );

	// Change status line color and text

	PS.statusColor( 0x185C7D );
	PS.statusText( "welcom to viddy gam :)" );
...
PS.touch = function( x, y, data, options ) {
	...
	if ( data === PS.color( x, y, r, g, b ) ) {
		next = PS.COLOR_WHITE;
	}
	else {
		next = PS.color( x, y, r, g, b );
	}
